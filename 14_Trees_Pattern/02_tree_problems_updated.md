...
### Category 1 (Extended II) — Traversal / Views / Advanced BFS ⭐ Striver
- [P55 — Pre, Inorder, Postorder in One Traversal](#problem-55--pre-inorder-postorder-in-one-traversal)
- [P56 — Right View of Binary Tree](#problem-56--right-view-of-binary-tree)
- [P57 — Bottom View of Binary Tree](#problem-57--bottom-view-of-binary-tree)
- [P58 — Top View of Binary Tree](#problem-58--top-view-of-binary-tree)
- [P59 — Vertical Order Traversal of Binary Tree](#problem-59--vertical-order-traversal-of-binary-tree)
- [P60 — Boundary Traversal of Binary Tree](#problem-60--boundary-traversal-of-binary-tree)
- [P61 — Print Root to Leaf Paths in Binary Tree](#problem-61--print-root-to-leaf-paths-in-binary-tree)
- [P62 — Minimum Time to Burn Binary Tree from a Node](#problem-62--minimum-time-to-burn-binary-tree-from-a-node)
- [P63 — Count Total Nodes in a Complete Binary Tree](#problem-63--count-total-nodes-in-a-complete-binary-tree)
- [P64 — Morris Preorder Traversal](#problem-64--morris-preorder-traversal)

### Category 3 (BST Extended II) — BST Operations ⭐ Striver
- [P65 — Floor and Ceil in a BST](#problem-65--floor-and-ceil-in-a-bst)
- [P66 — Delete a Node in BST](#problem-66--delete-a-node-in-bst)
- [P67 — Kth Largest Element in BST](#problem-67--kth-largest-element-in-bst)
...
## Category 1 (Extended) — Traversal / Views / BFS Advanced

> **Striver topics covered here:** Boundary Traversal, Vertical Order Traversal, Top View, Bottom View, Right/Left View, Print Root to Leaf Path, Pre+Post+Inorder in one traversal, Morris Preorder, Burn Tree, Count nodes in Complete BT

---

### Problem 55 — Pre, Inorder, Postorder in One Traversal
**No direct LeetCode | Difficulty: Medium | Company: Amazon, Google | Category: Traversal**

> Print preorder, inorder, and postorder traversals in a single DFS pass using a state variable per node.

#### Core insight

Each node is visited 3 times. Use a stack of `(node, state)` pairs:
- state=1 → preorder visit, push state=2, go left
- state=2 → inorder visit, push state=3, go right
- state=3 → postorder visit, pop (done)
...
public void allTraversals(TreeNode root) {
    ...
    Deque<int[]> stack = new ArrayDeque<>(); // [nodeRef encoded, state] — use Object stack below
...
### Problem 56 — Right View of Binary Tree
**LeetCode #199 | Difficulty: Medium | Company: Amazon, Facebook | Category: BFS / DFS**

> Return the values of the nodes visible when looking at the tree from the right side.

#### Core insight

BFS: last node of each level is visible from right. DFS: visit right child first; first node seen at each depth is the right view node.
...
public List<Integer> rightSideView(TreeNode root) {
    ...
    while (!q.isEmpty()) {
        ...
        for (int i = 0; i < size; i++) {
            TreeNode node = q.poll();
            if (i == size - 1) res.add(node.val);   // last node of level = rightmost
            if (node.left  != null) q.offer(node.left);
            if (node.right != null) q.offer(node.right);
        }
    }
...
// Left view: just change i == size - 1  →  i == 0
```
...
### Problem 57 — Bottom View of Binary Tree
**GeeksforGeeks | Difficulty: Medium | Company: Amazon, Flipkart | Category: BFS + Map**

> Print the bottom view: for each horizontal distance (HD), print the last node seen at that HD during BFS.

#### Core insight

BFS with horizontal distance tracking. `(node, hd)` in queue. For each HD, the latest node encountered (deepest level) is the bottom view node. Use a TreeMap to keep HDs sorted.
...
public List<Integer> bottomView(TreeNode root) {
    ...
    Map<Integer, Integer> hdMap = new TreeMap<>();  // HD → node val (overwrite = deepest)
    Queue<Object[]> q = new LinkedList<>();         // [TreeNode, hd]
...
Note: hd=0 → 5 overwrites 1 (deeper level)
```

---

### Problem 58 — Top View of Binary Tree
**GeeksforGeeks | Difficulty: Medium | Company: Amazon, Microsoft | Category: BFS + Map**

> Print the top view: for each horizontal distance (HD), print the FIRST node seen (shallowest level).

#### Core insight

Same as bottom view but **do NOT overwrite** once an HD is already recorded. The first node encountered at each HD during BFS is the top view node.
...
public List<Integer> topView(TreeNode root) {
    ...
    while (!q.isEmpty()) {
        ...
        hdMap.putIfAbsent(hd, node.val);   // only first node at each HD (top view)
...
Top view: [4, 2, 1, 3, 6]
Note: hd=0 → 1 stays (not overwritten by 5)
```

---

### Problem 59 — Vertical Order Traversal of Binary Tree
**LeetCode #987 | Difficulty: Hard | Company: Amazon, Facebook, Google | Category: BFS + Map**

> Return nodes column by column (left to right). Within same column and row, sort by value.

#### Core insight

BFS with `(node, row, col)`. Group by col → TreeMap. Within same col, group by row → TreeMap. Within same col+row, sort by value → PriorityQueue.
...
### Problem 60 — Boundary Traversal of Binary Tree
**GeeksforGeeks | Difficulty: Medium | Company: Amazon, Samsung | Category: DFS**

> Print left boundary (top-down, excluding leaves), all leaves (left to right), right boundary (bottom-up, excluding leaves).

#### Core insight

Three separate passes:
1. Left boundary: go left preferring left child, skip leaves
2. Leaf nodes: standard DFS, add when both children are null
3. Right boundary: go right preferring right child, skip leaves, then reverse
...
### Problem 61 — Print Root to Leaf Paths in Binary Tree
**GeeksforGeeks / LeetCode #257 variant | Difficulty: Easy | Category: DFS**

> Print all root-to-leaf paths.

#### Core insight

DFS with backtracking. Maintain current path, print when leaf is reached.
...
### Problem 62 — Minimum Time to Burn Binary Tree from a Node
**GeeksforGeeks | Difficulty: Hard | Company: Amazon, Flipkart | Category: BFS + Graph**

> Given a target node, find the minimum time to burn the entire tree if fire starts at that node. Fire spreads to adjacent nodes (parent, left child, right child) each second.

#### Core insight

Step 1: Build a parent map (each node → parent). Step 2: BFS from target node treating tree as undirected graph. Answer = BFS levels needed to visit all nodes.
...
```
    ...
    / \
   4   5
Target = 2, fire spreads each second:
t=0: {2}
t=1: {4, 5, 1}
t=2: {3}
Answer = 2 ✓
...
### Problem 63 — Count Total Nodes in a Complete Binary Tree
**LeetCode #222 | Difficulty: Medium | Company: Amazon, Google | Category: Properties**

> Count total nodes in a complete binary tree more efficiently than O(n).

#### Core insight

In a complete binary tree: find left height and right height by going all the way left/right. If equal → perfect binary tree → `2^h - 1` nodes. If not equal → recurse on both halves. Each call either terminates in O(log n) or recurses on one side. Time: O(log²n).
...
leftH = 3, rightH = 2 → not equal → recurse
left subtree (root=2): leftH=2, rightH=2 → perfect → 3 nodes
right subtree (root=3): leftH=2, rightH=1 → recurse → 2 nodes
Total = 1 + 3 + 2 = 6 ✓
```

---

### Problem 64 — Morris Preorder Traversal
**No LeetCode | Difficulty: Medium | Category: Morris / Constant Space**

> Preorder traversal in O(1) space (no stack, no recursion) using Morris threading.

#### Core insight

Morris Preorder = same threading as Morris Inorder, but **print node BEFORE going left** (first visit). Thread: find inorder predecessor of current node, link its right to current. On second visit, unlink and go right.
...
public List<Integer> morrisPreorder(TreeNode root) {
    ...
    while (curr != null) {
        if (curr.left == null) {
            ...
            if (pred.right == null) {
                res.add(curr.val);   // FIRST visit: print BEFORE going left (preorder)
                pred.right = curr;   // thread
                curr = curr.left;
            } else {
                pred.right = null;   // remove thread (second visit: don't print again)
                curr = curr.right;
            }
        }
    }
...
## Category 3 (BST Extended II) — BST Operations

> **Striver topics covered here:** Floor and Ceil in BST, Delete a node in BST, Kth Largest in BST, Inorder Successor/Predecessor, BST Iterator, Largest BST in Binary Tree

---

### Problem 65 — Floor and Ceil in a BST
**GeeksforGeeks / LeetCode #700 variant | Difficulty: Medium | Company: Amazon | Category: BST Search**

> Floor = largest value ≤ target. Ceil = smallest value ≥ target.

#### Core insight

BST property: if `node.val <= target`, it could be the floor — save it and go right to find closer. If `node.val > target`, go left.
...
public int floorBST(TreeNode root, int target) {
    ...
    while (root != null) {
        ...
        if (root.val < target) { floor = root.val; root = root.right; }  // potential floor, try closer
        else                     root = root.left;
    }
    return floor;
}
...
public int ceilBST(TreeNode root, int target) {
    ...
    while (root != null) {
        if (root.val == target) return root.val;
        if (root.val > target) { ceil = root.val; root = root.left; }    // potential ceil, try closer
...
### Problem 66 — Delete a Node in BST
**LeetCode #450 | Difficulty: Medium | Company: Amazon, Microsoft | Category: BST**

> Delete a node with given key from BST. Return the updated root.
...
3. Node has two children → replace value with **inorder successor** (smallest in right subtree), then delete successor from right subtree
...
### Problem 67 — Kth Largest Element in BST
**GeeksforGeeks | Difficulty: Medium | Company: Amazon | Category: BST Inorder**

> Find the Kth largest element in a BST.

#### Core insight

Reverse inorder traversal (Right → Root → Left) of BST gives nodes in descending order. Stop at the Kth node.
...
### Problem 68 — Inorder Successor and Predecessor in BST
**GeeksforGeeks | Difficulty: Medium | Company: Microsoft, Amazon | Category: BST**

> Find the inorder successor (next larger) and inorder predecessor (next smaller) of a given key in a BST.

#### Core insight

**Successor**: the leftmost node in the right subtree, OR the last ancestor where we went left.
**Predecessor**: the rightmost node in the left subtree, OR the last ancestor where we went right.
...
public TreeNode inorderSuccessor(TreeNode root, TreeNode p) {
    ...
    while (root != null) {
        if (p.val < root.val) {
            successor = root;       // candidate: could be successor
            root = root.left;       // try to find closer one
        } else {
...
public TreeNode inorderPredecessor(TreeNode root, TreeNode p) {
    ...
    while (root != null) {
        if (p.val > root.val) {
            predecessor = root;     // candidate
            root = root.right;      // try to find closer one
        } else {
            root = root.left;
        }
    }
...
inorderSuccessor(8)   = 12
inorderPredecessor(12) = 8
```

---

### Problem 69 — BST Iterator
**LeetCode #173 | Difficulty: Medium | Company: LinkedIn, Google, Amazon | Category: BST**

> Implement an iterator over a BST that gives the next smallest element in O(1) average time and O(h) space.

#### Core insight

Simulate iterative inorder using a stack. `next()` pops from stack, then pushes all left children of the right subtree. Each node is pushed/popped exactly once → amortized O(1).
...
hasNext() after 20 → false ✓
```

---

### Problem 70 — Largest BST in Binary Tree
**LeetCode #333 / GeeksforGeeks | Difficulty: Hard | Company: Amazon, Google | Category: BST Validation**

> Find the size of the largest subtree which is also a BST.

#### Core insight

Bottom-up DFS. Each call returns `(isBST, size, min, max)`. A subtree is BST if both children are BSTs AND `leftMax < root.val < rightMin`.
...
// returns int[]{size, min, max}; size=-1 if not a BST
private int[] dfs(TreeNode node) {
    ...
    // left[2] = max of left subtree, right[1] = min of right subtree
...
Largest BST: subtree rooted at 5 → [1, 5, 8], size = 3
(15→7 violates BST property of parent)
Output: 3 ✓
```
